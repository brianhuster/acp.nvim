...
function SchemaConverter:json_type_to_lua(schema_type)
    if type(schema_type) == "table" then
        ...
        for _, t in ipairs(schema_type) do
            if t == "null" then table.insert(types, "nil")
            elseif t == "string" then table.insert(types, "string")
            elseif t == "number" or t == "integer" then table.insert(types, "number")
            elseif t == "boolean" then table.insert(types, "boolean")
            elseif t == "array" then table.insert(types, "table")
            elseif t == "object" then table.insert(types, "table")
            else table.insert(types, t) end
        end
        return table.concat(types, "|")
    end

    if schema_type == "null" then return "nil"
    elseif schema_type == "string" then return "string"
    elseif schema_type == "number" or schema_type == "integer" then return "number"
    elseif schema_type == "boolean" then return "boolean"
    elseif schema_type == "array" then return "table"
    elseif schema_type == "object" then return "table"
    else return schema_type or "any" end
end
...
function SchemaConverter:process_enum(schema)
    if schema.const then
        if type(schema.const) == "string" then return '"' .. schema.const .. '"'
        else return tostring(schema.const) end
    end
    if schema.enum then
        local values = {}
        for _, v in ipairs(schema.enum) do
            if type(v) == "string" then table.insert(values, '"' .. v .. '"')
            else table.insert(values, tostring(v)) end
        end
...
function SchemaConverter:process_property(prop_name, prop_schema, required)
    ...
    if prop_schema["$ref"] then
        ...
        if ref then lua_type = "acp." .. ref end
    ...
    elseif prop_schema.anyOf then
        ...
        for _, schema in ipairs(prop_schema.anyOf) do
            if schema["$ref"] then
                ...
                if ref then table.insert(types, "acp." .. ref) end
    ...
    if not required then lua_type = lua_type .. "?" end
    local result = "---@field " .. prop_name .. " " .. lua_type
    if description then result = result .. " " .. description:gsub("\n", " ") end
    return result
end
...
function SchemaConverter:process_type(type_name, type_schema)
    if self.processed_types[type_name] then return end
    self.processed_types[type_name] = true

    if type_schema.description then
        self:add_line("")
        local desc = vim.split(type_schema.description, "\n")
        for _, line in ipairs(desc) do self:add_line("---" .. line) end
    else self:add_line("") end
    ...
    if union_key then
        ...
        for _, variant in ipairs(variants) do if not variant.const then all_const = false break end end
        if all_const then
            local values = {}
            for _, variant in ipairs(variants) do table.insert(values, self:process_enum(variant)) end
    ...
    if type_schema.type and not type_schema.properties and not type_schema.allOf then
        ...
        if type_schema.properties then
            local req = {}
            if type_schema.required then for _, f in ipairs(type_schema.required) do req[f] = true end end
